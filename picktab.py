...
class TestPickTab(unittest.TestCase):
    ...
    def run_picktab(self, path, gold):
        ...
        for y, r in enumerate(res):
            for x, c in enumerate(r):
                try:
                    self.assertGreater(ratio(res[y][x].lower(), gold[y][x].lower()), 0.97)
                except:
                    print(res[y][x], " <!=> ", gold[y][x])
                    raise

    def test_vs1(self):
        path = "./tests/picktab/Amazon-vs-Google.jpg.bw.png"
        gold = [['Amazon', 'Google'],
    ...
    def test_vs2(self):
        path = "./tests/picktab/AWS-VERSUS-Google-Cloud.jpg.bw.png"
    ...
    def test_vs3(self):
        path = "./tests/picktab/Anointing-vs-Holy-Spirit.jpg.bw.png"
    ...
    def test_VERSUS(self):
        path = "./tests/picktab/Capex-VERSUS-Opex.jpg.bw.png"
        gold = [['Meaning',
...
if __name__ == '__main__':
    t = PickTab(n_cols=4, min_col_width=300, min_row_width=60)
    unittest.main()

    t.analyse_pic_data(pic_input_dir, input_image_file_filter)
